...
export function generateFullProfileCode(
  ...
  if (preset === 'cpp_constexpr') {
    code = `// ==========================================================\n` +
      `// Generated by IL2CPP Memory Manager\n` +
...
export function generateScanHistoryCode(
  ...
  const dateStr = new Date(history.timestamp).toISOString();

  // Convert history items to WatchlistTargetItem-like objects
  const targetItems: WatchlistTargetItem[] = history.items.map((it, idx) => ({
    id: it.id || `hist_${idx}`,
  ...
  if (preset === 'cpp_constexpr') {
    code = `// ==========================================================\n` +
      `// Generated by IL2CPP Memory Manager - Scan History Log\n` +
      `// Profile: ${history.profileName}\n` +
      `// Target: ${history.targetApp} (${history.sourceMode === 'live' ? 'Live Process' : 'dump.cs'})\n` +
      `// Scanned At: ${dateStr}\n` +
      `// Resolved: ${history.resolvedCount} / ${history.totalTargets}\n` +
      `// Code Style: C++ (constexpr)\n` +
      `// ==========================================================\n\n` +
      `#pragma once\n` +
      `#include <cstdint>\n\n` +
      `namespace Offsets {\n` +

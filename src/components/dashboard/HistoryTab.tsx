...
export const HistoryTab: React.FC<HistoryTabProps> = ({
  ...
  return (
    <div className="max-w-5xl mx-auto w-full p-2 sm:p-3 flex flex-col gap-2 sm:gap-3 pb-12">
      <div className="flex flex-col gap-2 sm:gap-2.5 animate-in fade-in slide-in-from-bottom-2 duration-300">
        <div className="flex flex-col rounded-xl sm:rounded-2xl border border-[#2D2D30] overflow-hidden shadow-sm">
          <div className="flex items-center justify-between p-2 sm:p-2.5 bg-[#1E1E20] border-b border-[#2D2D30] gap-2">
            <div className="flex items-center gap-1.5 sm:gap-2">
              <div className="p-1 sm:p-1.2 rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 shrink-0">
                <History className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
              </div>
              <span className="font-semibold text-xs sm:text-sm text-white">Scan Logs</span>
              <span className="text-[9.5px] sm:text-xs font-mono px-1.5 py-0.2 rounded bg-indigo-500/10 text-indigo-300 border border-indigo-500/20">
                {scanHistory.length}
              </span>
            </div>
            ...
            <div className="flex items-center gap-1 sm:gap-1.5">
              <button
                ...
                className="flex items-center gap-1 px-2 sm:px-2.5 py-1 bg-[#262629] hover:bg-[#323236] text-[#A0A0A5] hover:text-white border border-[#353538] rounded-lg text-[10px] sm:text-xs font-semibold transition-colors shadow-sm"
              ...
              {scanHistory.length > 0 && (
                <button
                  ...
                  className="text-[10px] sm:text-xs font-semibold px-2 sm:px-2.5 py-1 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 rounded-lg transition-colors flex items-center gap-1"
                >
          ...
          <div
            ...
            } gap-1.5 sm:gap-2 p-1.5 sm:p-2 bg-[#141416]`}
          >
            {scanHistory.length === 0 ? (
              <div className="p-6 sm:p-8 text-center text-[#8E8E93] flex flex-col items-center col-span-full">
                <History className="w-7 h-7 sm:w-8 sm:h-8 text-[#3A3A3E] mb-1.5" />
              ...
              scanHistory.map((rec) => {
                ...
                return (
                  <div
                    ...
                    className={`${
                      isCompact ? 'p-1.5 sm:p-2 gap-1' : 'p-2 sm:p-2.5 gap-1 sm:gap-1.5'
                  ...
                  >
                    <div className="flex items-center justify-between text-xs gap-1.5">
                      <div className="flex items-center gap-1.5 min-w-0 flex-1">
                        <span className="font-semibold text-[11px] sm:text-xs text-indigo-300 group-hover:text-indigo-200 transition-colors truncate">
                          {rec.profileName}
                        </span>
                      </div>
                    ...
                    {/* Live Half Side Box: Resolution Rate + Active Format (Compact for Mobile & Tablet) */}
                    {cardViewSettings.showMetadata && (
                      <div className="flex items-center justify-between gap-1.5 pt-0.5">
                        <div className="inline-flex items-center rounded-md sm:rounded-lg bg-[#141416] border border-[#2F2F34] text-[8px] sm:text-[8.5px] font-mono overflow-hidden shadow-inner">
                          {/* Resolution Rate */}
                          <span className="flex items-center gap-1 px-1.5 py-0.5 bg-emerald-500/10 text-emerald-400 font-semibold border-r border-[#2F2F34]">
                            <span className="w-1.2 h-1.2 rounded-full bg-emerald-400"></span>
                          ...
                          {/* Active Format */}
                          <span className="flex items-center gap-1 px-1.5 py-0.5 text-indigo-300 font-medium">
                            <Code2 className="w-2.5 h-2.5 text-indigo-400 shrink-0" />
                            <span className="truncate max-w-[95px] sm:max-w-[130px]">{presetObj.label}</span>
                          </span>
                        </div>

                        <div className="flex items-center gap-1.5 ml-auto text-[8px] sm:text-[8.5px] text-[#6C6C70]">
                    ...
                    {cardViewSettings.showOffsetTags && (
                      <div className="flex flex-wrap gap-1 font-mono text-[7.5px] sm:text-[8.5px] pt-0.5">
